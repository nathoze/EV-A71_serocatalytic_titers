...
# TO DO: If we change only one FOI, then we need to compute only a few of the titer distribution 
titer_distribution <- function(all.parameters, indices.changes = 1:N.FOI){ # replace on specific indices
...
get_all_parameters <- function(params){
  ...
  all.parameters$titer.distribution = titer_distribution(all.parameters)
...
update_all_parameters <- function(old.all.parameters, new.param, updated_index){
  ...
  if(updated_index <= N.FOI){ # We change the foi and therefore only the corresponding matrix
    ...
    new.all.parameters$titer.distribution = titer_distribution(new.all.parameters)
  }  
